...
void Printer::homeZAxis() // Cartesian homing
{
    ...
    if ((MIN_HARDWARE_ENDSTOP_Z && Z_MIN_PIN > -1 && Z_HOME_DIR == -1) || (MAX_HARDWARE_ENDSTOP_Z && Z_MAX_PIN > -1 && Z_HOME_DIR == 1))
    {
		...
		transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
#endif
        PrintLine::moveRelativeDistanceInSteps(0, 0, axisStepsPerMM[Z_AXIS] * -ENDSTOP_Z_BACK_MOVE * Z_HOME_DIR, 0, homingFeedrate[Z_AXIS],true,false);
...
#elif HOMING_ORDER == HOME_ORDER_ZXYTZ || HOMING_ORDER == HOME_ORDER_XYTZ
{
    ...
    if(zaxis) {
        ...
        homeZAxis();
#endif
        Printer::moveToReal(IGNORE_COORDINATE,IGNORE_COORDINATE,ZHOME_HEAT_HEIGHT,IGNORE_COORDINATE,maxFeedrate[Z_AXIS]);
        Commands::waitUntilEndOfAllMoves();
#if ZHOME_HEAT_ALL
        for(int i = 0; i < NUM_EXTRUDER; i++) {
...
//#endif
    ...
    if(zaxis) {
        ...
        moveToReal(ZHOME_X_POS,ZHOME_Y_POS,IGNORE_COORDINATE,IGNORE_COORDINATE,maxFeedrate[Y_AXIS]); // correct rotation!
        Commands::waitUntilEndOfAllMoves();
#endif
		...
		moveToReal(IGNORE_COORDINATE, IGNORE_COORDINATE, ZHOME_HEAT_HEIGHT, IGNORE_COORDINATE, maxFeedrate[X_AXIS]); // correct rotation!
#if ZHOME_HEAT_ALL
        for(int i = 0; i < NUM_EXTRUDER; i++)
        ...
        for(int i = 0; i < NUM_EXTRUDER; i++)
			Extruder::setTemperatureForExtruder(actTemp[i],i,false, false);
#else
        Extruder::setTemperatureForExtruder(actTemp[Extruder::current->id], Extruder::current->id, false, false);
#endif
    }
}
#endif
